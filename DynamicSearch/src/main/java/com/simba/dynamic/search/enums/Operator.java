...
import com.simba.dynamic.search.VO.FilterRequest;
...
public enum Operator {
    ...
    LIKE {
        public <T> Predicate build(Root<T> root, CriteriaBuilder cb,
            ...
            return cb.and(
                    cb.like(cb.upper(key), "%" + request.getValue()
                            .toString().toUpperCase() + "%"), predicate);
        }
    },
    ...
    IN {
        public <T> Predicate build(Root<T> root, CriteriaBuilder cb,
            ...
            values.stream().map(value -> request.getFieldType().parse(value.toString()))
